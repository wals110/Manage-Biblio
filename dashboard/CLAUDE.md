...
- **Affectation en lot** (colonne Thèmes LLM) : checkboxes de multi-sélection + toolbar avec 2 actions :
  ...
  - **💡 Suggérer + Mapper** : `POST /api/taxonomy/mappings/suggest` (`suggest_mappings`, moteur **hybride** : KeywordClassifier déterministe gratuit d'abord, puis LLM mapper parallélisé borné `max_llm` pour les ambigus, no-op sans clé API) → panneau de revue (badge source déterministe/LLM/non-résolu, confiance, dossier éditable inline, non-résolus décochés) → Appliquer les acceptés en bulk-add
