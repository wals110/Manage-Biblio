# YAML Validator — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a standalone Python script that validates and auto-fixes `tests/functional/tests.yaml`, encoding all known shell pitfalls as deterministic rules.

**Architecture:** Single script `tests/functional/validate_tests.py` with a list of rule functions. Each rule returns findings (fixes, errors, warnings). The script applies fixes when `--fix` is passed, then reports remaining issues. Rules 6-7 load optional project context (tree.yaml, source code).

**Tech Stack:** Python 3.13, stdlib + pyyaml only

**Spec:** `docs/superpowers/specs/2026-04-06-yaml-validator-design.md`
...
```
tests/functional/
├── validate_tests.py    # NEW — the validator script
├── tests.yaml           # existing — file being validated
├── run_functional.py    # existing — runner (not modified)
└── reports/             # existing
```

---

### Task 1: Script skeleton + CLI + YAML loading

**Files:**
- Create: `tests/functional/validate_tests.py`

- [ ] **Step 1: Create the script with CLI parsing and YAML loading**
...
def iter_commands(data: dict):
    """Yield (location_str, command_str, container_obj, key) for all commands in the YAML."""
...
- [ ] **Step 2: Verify it runs (will fail on missing rule functions)**

```bash
uv run python tests/functional/validate_tests.py 2>&1 | head -5
```

Expected: `NameError: name 'rule_grep_c_echo' is not defined`

- [ ] **Step 3: Commit skeleton**

```bash
git add tests/functional/validate_tests.py
git commit -m "feat: add validate_tests.py skeleton with CLI and YAML loading"
```

---

### Task 2: Auto-fixable rules (1-4)

**Files:**
- Modify: `tests/functional/validate_tests.py`

- [ ] **Step 1: Add rule_grep_c_echo**
...
def rule_grep_c_echo(data: dict, fix: bool = False) -> list[Finding]:
    ...
    for loc, cmd, container, key in iter_commands(data):
        if 'grep' in cmd and re.search(pattern, cmd):
            new_cmd = re.sub(r'\|\|\s*echo\s+0', '; true', cmd)
            if fix:
...
- [ ] **Step 5: Test rules 1-4 on current tests.yaml**

```bash
uv run python tests/functional/validate_tests.py 2>&1 | head -20
```

Expected: should report issues (or none if tests.yaml is already clean from our manual fixes).

- [ ] **Step 6: Test --fix mode**

```bash
cp tests/functional/tests.yaml tests/functional/tests.yaml.bak
uv run python tests/functional/validate_tests.py --fix 2>&1
diff tests/functional/tests.yaml tests/functional/tests.yaml.bak
rm tests/functional/tests.yaml.bak
```
...
```bash
git add tests/functional/validate_tests.py
git commit -m "feat: add auto-fixable rules 1-4 (grep-c, missing-y, clean-all)"
```

---

### Task 3: Warning rules (5, 9, 10)

**Files:**
- Modify: `tests/functional/validate_tests.py`

- [ ] **Step 1: Add rule_absolute_paths**
...
- [ ] **Step 2: Add rule_dependencies**

```python
def rule_dependencies(data: dict) -> list[Finding]:
    """Rule 9: check requires/provides consistency"""
    findings = []
    all_provides: dict[str, str] = {}  # capability → series_id
    all_requires: dict[str, list[str]] = {}  # series_id → [capabilities]
...
- [ ] **Step 3: Add rule_blocking_commands**
...
```bash
uv run python tests/functional/validate_tests.py 2>&1
```
...
```bash
git add tests/functional/validate_tests.py
git commit -m "feat: add warning rules (absolute-paths, dependencies, blocking-commands)"
```

---

### Task 4: Structure validation rule (8)

**Files:**
- Modify: `tests/functional/validate_tests.py`

- [ ] **Step 1: Add rule_yaml_structure**
...
```bash
git add tests/functional/validate_tests.py
git commit -m "feat: add YAML structure validation rule"
```

---

### Task 5: Context-dependent rules (6, 7)

**Files:**
- Modify: `tests/functional/validate_tests.py`

- [ ] **Step 1: Add load_context and rule_paths_exist**

```python
# ── Runtime file patterns (don't flag these) ───────────────
RUNTIME_PATTERNS = {
    ".setup_output_", ".bench_", "report_"
}
...
def load_tree_yaml(profile: str = "test") -> list[str] | None:
    ...
    if not tree:
        return None
    # Flatten tree structure to list of paths
    paths = []
...
def resolve_variables(cmd: str, variables: dict) -> str:
    """Resolve ${VAR} in a command string."""
    result = cmd
    for _ in range(10):
...
def rule_paths_exist(data: dict) -> list[Finding]:
    """Rule 6: check that referenced paths exist in tree.yaml"""
    findings = []
    variables = data.get("variables", {})

    # Try to get profile name from variables
    profile = variables.get("PROF", "test")
    tree_paths = load_tree_yaml(profile)
    if tree_paths is None:
        return findings  # SKIP if tree.yaml not available

    # Extract folder paths from ls/find commands
    ...
    for loc, cmd, _, _ in iter_commands(data):
        ...
        if is_runtime_path(resolved):
            continue
        for pattern in (ls_pattern, find_pattern):
            for match in pattern.finditer(resolved):
                path = match.group(1).rstrip("/")
                # Check if this path references a project subfolder
                ...
                if biblio_test and path.startswith(biblio_test):
                    ...
                    if relative and "/" in relative:
                        # Check deepest subfolder against tree
...
def rule_report_names(data: dict) -> list[Finding]:
    ...
    for search_dir in (PROJECT_ROOT / "commands", PROJECT_ROOT / "lib"):
        ...
        for py_file in search_dir.glob("*.py"):
            ...
            # Also catch format: refine_{}.csv pattern
            for m in re.finditer(r"['\"](\w+)_\{\}\.csv['\"]", content):
                valid_prefixes.add(m.group(1))

    if not valid_prefixes:
        return findings  # SKIP if can't find source

    # Check YAML commands for report patterns
...
```bash
git add tests/functional/validate_tests.py
git commit -m "feat: add context-dependent rules (paths-exist, report-names)"
```

---

### Task 6: Output formatting (terminal + JSON)

**Files:**
- Modify: `tests/functional/validate_tests.py`

- [ ] **Step 1: Add print_report and print_json**
...
```bash
git add tests/functional/validate_tests.py
git commit -m "feat: add terminal and JSON output formatting for validator"
```

---

### Task 7: Update Skill 1 SKILL.md + final validation

**Files:**
- Modify: `~/.claude/skills/functional-test-plan/SKILL.md`

- [ ] **Step 1: Replace CRITICAL rules with validator instruction**

In the SKILL.md, replace the entire "Important" section (from `## Important` to before `## CRITICAL — Do Not Forget the Markdown`) with:
...
- Use `provides:` / `requires:` for logical dependencies.
- The `session.pre_run` should contain the global reset command.
- Mark checks as `mode: manual_check` only when automated verification is genuinely impossible.

## Post-Generation Validation

After generating tests.yaml, ALWAYS run the validator:

```bash
uv run python tests/functional/validate_tests.py --fix
```

The validator auto-fixes known shell pitfalls (grep -c, missing -y, clean all) and reports errors it cannot fix (wrong paths, wrong report names, structural issues).

If ERRORS remain after auto-fix, correct them manually before generating the Markdown.

**The validator is the source of truth for shell command conventions — trust it over any instructions in this file.**
```

- [ ] **Step 2: Run the full validation on current tests.yaml**

```bash
uv run python tests/functional/validate_tests.py
```

Expected: 0 errors, 0 fixes (tests.yaml should be clean from manual fixes), possibly some warnings.

- [ ] **Step 3: Run the runner dry-run to confirm nothing broke**

```bash
uv run python tests/functional/run_functional.py --dry-run 2>&1 | tail -5
```

Expected: `DRY RUN complete — nothing executed.`

- [ ] **Step 4: Commit everything**

```bash
git add tests/functional/validate_tests.py
git commit -m "feat: complete YAML validator with 10 rules, update Skill 1 to use it

- 4 auto-fixable rules (grep-c, missing-y, clean-all)
- 3 warning rules (absolute-paths, dependencies, blocking-commands)
- 3 error rules (yaml-structure, paths-exist, report-names)
- Terminal + JSON output
- Skill 1 SKILL.md simplified: CRITICAL rules replaced by validator call"
```
